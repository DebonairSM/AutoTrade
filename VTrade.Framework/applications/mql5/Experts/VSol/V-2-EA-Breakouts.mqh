    ...
    bool Init(int lookbackPeriod, double minStrength, double touchZone, int minTouches, bool showDebugPrints)
    {
        ...
        else
        {
            ...
            if(touchZone == 0 || touchZone > 1.0)  // Added check for unreasonably large values
            {
                ...
                Print("ℹ️ [Init] Using default forex touch zone: ", touchZone, " (", touchZone/_Point, " pips)");
            }
            else
            {
                Print("ℹ️ [Init] Using provided forex touch zone: ", touchZone, " (", touchZone/_Point, " pips)");
            }
        }
