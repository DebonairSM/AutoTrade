...
namespace VSol
{
    ...
    public:
        ...
        ~CV2EABreakouts(void)
        {
            // Call cleanup method to ensure proper resource deallocation
            Cleanup();
        }
        ...
        void Cleanup()
        {
            ...
            ArrayFree(m_recentBreaks);
            ArrayFree(m_recentBreakTimes);
            ...
            if(m_showDebugPrints)
                DebugPrint("🧹 Cleaned up all resources and chart objects");
        }

        void ClearChartObjects()
        {
            // Clear all existing chart lines with proper error handling
            for(int i = 0; i < ArraySize(m_chartLines); i++)
            {
                if(m_chartLines[i].name != NULL && m_chartLines[i].name != "")
                {
                    if(!ObjectDelete(0, m_chartLines[i].name))
                    {
                        ...
                        {
                            DebugPrint(StringFormat("❌ Failed to delete line %s, error: %d", 
                                m_chartLines[i].name, error));
                        }
                    }
                }
            }
