...
private:
    ...
    //--- Moving Average Settings and Cache
    static int      m_fastMA;                 // Fast MA period (default 20)
    static int      m_slowMA;                 // Slow MA period (default 50)
    static int      m_trendMA;                // Trend MA period (default 200)
    static ENUM_MA_METHOD m_maMethod;         // MA calculation method
    static ENUM_TIMEFRAMES m_maTF;            // MA timeframe
    static int      m_maHandle[];             // Array of MA indicator handles
    static datetime m_lastMAUpdate;           // Last MA cache update time
    static double   m_cachedMA[];             // Cached MA values
...
public:
    ...
    static bool InitMovingAverages(int fastPeriod = 20, int slowPeriod = 50, int trendPeriod = 200,
                                 ENUM_MA_METHOD method = MODE_SMA, ENUM_TIMEFRAMES timeframe = PERIOD_CURRENT)
    {
        // Store settings
        m_fastMA = fastPeriod;
        m_slowMA = slowPeriod;
        m_trendMA = trendPeriod;
        m_maMethod = method;
        m_maTF = timeframe;
        
        // Initialize cache
        ArrayResize(m_cachedMA, 3);  // [fast, slow, trend]
        ArrayInitialize(m_cachedMA, 0);
        m_lastMAUpdate = 0;
        
        // Create MA indicators
        ArrayResize(m_maHandle, 3);
        
        // Create handles for each MA
        m_maHandle[0] = iMA(_Symbol, m_maTF, m_fastMA, 0, m_maMethod, PRICE_CLOSE);
        m_maHandle[1] = iMA(_Symbol, m_maTF, m_slowMA, 0, m_maMethod, PRICE_CLOSE);
        m_maHandle[2] = iMA(_Symbol, m_maTF, m_trendMA, 0, m_maMethod, PRICE_CLOSE);
        
        // Verify handles
        for(int i = 0; i < 3; i++)
        {
            if(m_maHandle[i] == INVALID_HANDLE)
            {
                CV2EAUtils::LogError(StringFormat("Failed to create MA indicator handle %d", i));
                return false;
            }
        }
    ...
    static void ReleaseMovingAverages()
    {
        for(int i = 0; i < ArraySize(m_maHandle); i++)
        {
            if(m_maHandle[i] != INVALID_HANDLE)
            {
                IndicatorRelease(m_maHandle[i]);
                m_maHandle[i] = INVALID_HANDLE;
            }
        }
    }
    
    static double GetMovingAverage(ENUM_MA_TYPE maType, int shift = 0)
    {
        // Determine which MA to use
        int index = -1;
        switch(maType)
        {
            case MA_FAST:  index = 0; break;
            case MA_SLOW:  index = 1; break;
            case MA_TREND: index = 2; break;
            default: return 0;
        }
        
        // Check if we need to update cache
        datetime current_time = iTime(_Symbol, m_maTF, shift);
        if(current_time != m_lastMAUpdate || m_cachedMA[index] == 0)
        {
            double ma_value[];
            if(CopyBuffer(m_maHandle[index], 0, shift, 1, ma_value) <= 0)
            {
                CV2EAUtils::LogError(StringFormat("Failed to copy MA data for type %s", EnumToString(maType)));
                return 0;
            }
            
            m_cachedMA[index] = ma_value[0];
            m_lastMAUpdate = current_time;
        }
        
        return m_cachedMA[index];
    }
    
    static bool IsTrendBullish()
    {
        double fastMA = GetMovingAverage(MA_FAST);
        double slowMA = GetMovingAverage(MA_SLOW);
        double trendMA = GetMovingAverage(MA_TREND);
        
        return fastMA > slowMA && slowMA > trendMA;
    }
    
    static bool IsTrendBearish()
    {
        double fastMA = GetMovingAverage(MA_FAST);
        double slowMA = GetMovingAverage(MA_SLOW);
        double trendMA = GetMovingAverage(MA_TREND);
    ...
    static bool IsMAAligned(bool bullish)
    {
        return bullish ? IsTrendBullish() : IsTrendBearish();
    }
    
    static bool IsPriceAboveMA(double price, ENUM_MA_TYPE maType, int shift = 0)
    {
        double ma = GetMovingAverage(maType, shift);
        return ma != 0 && price > ma;
    }
    
    static bool IsPriceBelowMA(double price, ENUM_MA_TYPE maType, int shift = 0)
    {
        double ma = GetMovingAverage(maType, shift);
        return ma != 0 && price < ma;
    }
                 ...
                 int minTouches, bool showDebugPrints)
    {
        ...
        string timeframeInfo = StringFormat(
            ...
            m_slowMA,
            m_trendMA,
            EnumToString(m_maMethod),
            EnumToString(m_maTF)
        );
        
        CV2EAUtils::LogInfo(timeframeInfo);
...
int      CV2EAMarketData::m_fastMA = 20;
int      CV2EAMarketData::m_slowMA = 50;
int      CV2EAMarketData::m_trendMA = 200;
ENUM_MA_METHOD CV2EAMarketData::m_maMethod = MODE_SMA;
ENUM_TIMEFRAMES CV2EAMarketData::m_maTF = PERIOD_CURRENT;
int      CV2EAMarketData::m_maHandle[];
datetime CV2EAMarketData::m_lastMAUpdate = 0;
double   CV2EAMarketData::m_cachedMA[];
...
void CV2EAMarketData::Init(bool showDebugPrints=false)
{
    ...
    // Initialize MAs with default values
    InitMovingAverages();
}
